...
const pb_evaluateSyntax = async (string, element) => {
    ...
    let pb_codeEvaluationString = await asyncReplace(string, /\${`([\s\S]*?)`}/g, async (match, code) => {
        try {
            ...
            if (result !== undefined) {
                return String('<span code="' + await pb_encrypt(pixelbite.version, code) + '"></span>' + result + '');
            } else {
                return "";
            }
        } catch (error) {
              ...
              category = line.trim().replaceAll('[', '').replaceAll(']', '')
                ...
                try {
                    if(category === "information" || category === "informations") {
                        ...
                        if ((variable === "scripts" || variable === "configs") && value) {
...
window.addEventListener('resize', () => {pb_classGenerator()});const class_library = [
    ['c', 'color'],
    ['p', 'padding'],
    ['pos', 'position'],
    ['o', 'opacity'],
